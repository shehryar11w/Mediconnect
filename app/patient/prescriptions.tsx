...
const PrescriptionsScreen = () => {
    ...
    const fetchPrescriptions = useCallback(async () => {
        try {
            setIsLoading(true);
            const response =
                await prescriptionService.PatientGetAllPrescriptions(
                    parseInt(currentUser.id),
                );
            console.log(response.data);
            if (response.success) {
                const formattedPrescriptions = response.data.map(
                    (prescription: any) => {
                        ...
                        const endDate = new Date(
                            startDate.getTime() +
                                prescription.PrescriptionDuration *
                                    24 *
                                    60 *
                                    60 *
                                    1000,
        ...
        >
            ...
            <View style={styles.prescriptionDetails}>
                <View style={styles.detailRow}>
                    ...
                    />
                    <ThemedText
                        style={[styles.detailText, { color: colors.textLight }]}
                    >
