...
const NotificationsScreen = () => {
  ...
  const notifications: Notification[] = [
    ...
    },
    {
      ...
      message: 'Dr. Mike Johnson has prescribed new medication. Tap to view details.',
      type: 'Prescription',
      date: '1 day ago',
      isRead: true,
      action: {
    ...
    },
    {
      ...
      message: 'Your recent blood test results are now available. Tap to view.',
      type: 'Report',
      date: '2 days ago',
      isRead: true,
      action: {
  ...
  const renderNotification = (notification: Notification) => (
    <TouchableOpacity
      ...
      style={[
        ...
        !notification.isRead && { borderLeftColor: colors.patientPrimary, borderLeftWidth: 4 }
      ]}
      // onPress={() => handleNotificationPress(notification)}
    >
      <View style={styles.notificationHeader}>
        <View style={[styles.iconContainer, { backgroundColor: getTypeColor(notification.type) + '20' }]}>
          <Ionicons name={getTypeIcon(notification.type)} size={24} color={getTypeColor(notification.type)} />
        </View>
        <View style={styles.notificationInfo}>
          <ThemedText style={styles.notificationTitle}>{notification.title}</ThemedText>
          <ThemedText style={[styles.notificationDate, { color: colors.textLight }]}>
            {notification.date}
          </ThemedText>
        </View>
        {!notification.isRead && (
          <View style={[styles.unreadDot, { backgroundColor: colors.patientPrimary }]} />
        )}
      </View>
    ...
    <ScrollView style={[globalStyles.container, styles.container]}>
      ...
      <View style={styles.tabContainer}>
        ...
        <TouchableOpacity
          style={[
            styles.tab,
            selectedTab === 'unread' && { borderBottomColor: colors.patientPrimary }
          ]}
        ...
        >
          <ThemedText
            style={[
              styles.tabText,
              selectedTab === 'unread' && { color: colors.patientPrimary }
            ]}
          >
            Unread
          </ThemedText>
