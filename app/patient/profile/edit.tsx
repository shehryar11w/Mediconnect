...
const EditProfileScreen = () => {
      ...
      try {
        ...
        if (patientDetails?.data) {
          setProfile({
              ...
              bloodGroup: patientDetails.data.details.PatientBloodGroup,
              emergencyContactName: patientDetails.data.details.PatientEmergencyContact[0]?.name || '',
              emergencyContactPhone: patientDetails.data.details.PatientEmergencyContact[0]?.phoneNumber || '',
              medicalRecord: patientDetails.data.details.PatientMedicalRecord,
  ...
  const handleSave = async () => {
    ...
    if (profile.emergencyContactPhone && !phoneRegex.test(profile.emergencyContactPhone)) {
    ...
    try {
      ...
      const updatedProfile = await detailsService.updatePatientDetails(currentUser?.id, {
    ...
    <ScrollView style={[globalStyles.container, styles.container]}>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.inputContainer, { backgroundColor: colors.background }]}>
          <TextInput
            ...
            placeholderTextColor={colors.textLight}
            value={profile?.emergencyContactName ? profile?.emergencyContactName : ''}
          ...
          <TextInput
            ...
            placeholderTextColor={colors.textLight}
            value={profile?.emergencyContactPhone ? profile?.emergencyContactPhone : ''}
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity 
            style={[styles.addButton, { backgroundColor: colors.patientPrimary }]}
            onPress={() => {
              setProfile({
                ...profile,
        ...
        <View style={[styles.inputContainer, { backgroundColor: colors.background }]}>
          {profile?.medicalRecord?.map((condition: string, index: number) => (
            <View key={`condition-${index}`} style={styles.medicalItemContainer}>
              <TextInput
                style={[styles.input, styles.medicalInput, { color: colors.text }]}
                placeholder="Medical Condition"
                placeholderTextColor={colors.textLight}
                value={condition}
                onChangeText={(text) => {
                  const newConditions = [...profile.medicalRecord];
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  const newConditions = profile.medicalRecord
                    ...
                    .filter((condition: string) => condition && condition.trim() !== '');
                  setProfile({
                    ...profile,
                    medicalRecord: newConditions
                  });
