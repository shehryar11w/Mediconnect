...
const BookAppointmentScreen = () => {
    ...
    const generateAvailableSlots = (date: Date) => {
        ...
        const [startHour, startMinute] =
            selectedDoctor.DoctorWorkingHours[0].start.split(":").map(Number);
        const [endHour, endMinute] = selectedDoctor.DoctorWorkingHours[0].end
            .split(":")
            .map(Number);
        ...
        for (
            ...
            timeInMinutes += selectedDuration
        ) {
    ...
    const getAvailableSlots = (date: Date) => {
        ...
        const dateStr = `${year}-${month}-${day}`;
        const bookedSlots =
            selectedDoctor.DoctorAppointments?.[dateStr]?.slots || [];
    ...
    const getMarkedDates = () => {
        ...
        while (currentDate <= twoMonthsLater) {
            ...
            const isUnavailable =
                selectedDoctor.DoctorUnavailableDays.includes(dateStr);
        ...
        return (
            ...
            >
                <View style={styles.timeSlotsHeader}>
                    <ThemedText style={styles.timeSlotsTitle}>
                ...
                >
                    {slots.map((slot: string, index: number) => {
                        ...
                        const isSelected =
                            selectedTime.getHours() === hour24 + 5 &&
        ...
        try {
            ...
            if (response.success) {
                notificationService.scheduleLocalNotification(
                    ...
                    {
                        appointmentId: response.data.appointmentId as string,
                        ...
                        >
                            <View style={styles.costRow}>
                                <ThemedText style={styles.costLabel}>
                                    Base Consultation (30 min)
                                </ThemedText>
                                <ThemedText style={styles.costValue}>
                                    PKR {selectedDoctor.DoctorBaseRate}
                                </ThemedText>
                            </View>
                            {selectedDuration !== 30 && (
                                <View style={styles.costRow}>
                                    <ThemedText style={styles.costLabel}>
                                        Duration Adjustment ({selectedDuration}{" "}
                                        min)
                                    </ThemedText>
                                    <ThemedText style={styles.costValue}>
                                        {selectedDuration > 30 ? "+" : "-"} PKR{" "}
                                        {Math.abs(
                                            Math.round(
                                                selectedDoctor.DoctorBaseRate *
                                                    (selectedDuration / 30 - 1),
                                            ),
                                        )}
                                    </ThemedText>
                                </View>
