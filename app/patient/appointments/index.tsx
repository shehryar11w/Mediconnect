...
const AppointmentsScreen = () => {
  ...
  const fetchAppointments = async () => {
    const res = await appointmentService.getPatientAppointments(currentUser?.id);
    console.log('appointments', res.data);
    setAppointments(res.data.map((appointment: any) => ({
      AppointmentId: appointment.AppointmentId,
      DoctorName: appointment.doctorName,
      DoctorSpecialization: appointment.doctorSpecialization,
      AppointmentDate: appointment.AppointmentStartDateTime.split('T')[0],
      AppointmentTime: appointment.AppointmentStartDateTime.split('T')[1].split('.')[0].split(':').slice(0, 2).join(':'),
  ...
  const filteredAppointments = appointments.filter(appointment => {
    if (activeTab === 'upcoming') {
      return appointment.AppointmentStatus === 'pending' || appointment.AppointmentStatus === 'rescheduled';
    } else {
      return appointment.AppointmentStatus === 'completed' || appointment.AppointmentStatus === 'cancelled';
    }
  });
  ...
  const confirmCancelAppointment = async () => {
    if (selectedAppointment) {
      ...
      try{
        const res = await appointmentService.cancelPatientAppointment(selectedAppointment.AppointmentId);
        if(res.success){
          Alert.alert(
          ...
          notificationService.scheduleLocalNotification(
            'Appointment Cancelled',
            'Your appointment has been cancelled successfully.',
            { appointmentId: selectedAppointment.AppointmentId as string, type: 'cancelled_appointment' }
          );
  ...
  const renderAppointment = ({ item, key }: { item: Appointment, key: number }) => (
    <View
    ...
    >
      <View style={styles.appointmentHeader}>
        ...
        >
          <ThemedText
            style={[styles.statusText, { color: getStatusColor(item.AppointmentStatus) }]}
          >
      ...
      <View style={styles.appointmentDetails}>
        <View style={styles.detailItem}>
          ...
          <ThemedText style={styles.detailText}>{item.AppointmentDate}</ThemedText>
        </View>
        <View style={styles.detailItem}>
          <Ionicons name="time" size={20} color={colors.patientPrimary} />
      ...
      {(item.AppointmentStatus.toLowerCase() === 'pending' || item.AppointmentStatus.toLowerCase() === 'rescheduled') && (
        <View style={styles.actionButtons}>
          ...
          <TouchableOpacity
            style={[styles.actionButton, { backgroundColor: colors.patientPrimary + '20' }]}
            onPress={() => handleRescheduleAppointment(item)}
          >
            <Ionicons name="calendar" size={20} color={colors.patientPrimary} />
            <ThemedText style={[styles.actionButtonText, { color: colors.patientPrimary }]}>
              Reschedule
            </ThemedText>
          </TouchableOpacity>
        </View>
  ...
  return (
    <ScrollView style={[globalStyles.container, styles.container, { backgroundColor: colors.background }]} refreshControl={
      <RefreshControl
      ...
      >
        <View style={styles.header}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => router.push('/patient/appointments/book' as any)}
          >
          ...
          >
            <ThemedText
              style={[
                styles.tabText,
                {
                  color: activeTab === 'upcoming' ? colors.patientPrimary : colors.background,
                },
              ]}
            >
              Upcoming
          ...
          >
            <ThemedText
              style={[
                styles.tabText,
                {
                  color: activeTab === 'past' ? colors.patientPrimary : colors.background,
                },
              ]}
            >
              Past
      ...
      {filteredAppointments.length === 0 && (
        <View style={styles.noAppointmentsContainer}>
          <ThemedText style={styles.noAppointmentsText}>No appointments found</ThemedText>
        </View>
      )}  
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.modalButtons}>
              ...
              >
                <ThemedText style={[styles.modalButtonText, { color: colors.error }]}>
                  No, Keep It
                </ThemedText>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <ThemedText style={[styles.modalButtonText, { color: colors.background }]}>
                  Yes, Cancel
                </ThemedText>
              </TouchableOpacity>
            </View>
