...
import { ChatDoctor, chatService, Message } from '../../../../services/chatService';
...
const ChatDetailScreen = () => {
  ...
  const fetchChatDetails = async () => {
    try {
      ...
      const doc = {
        ...
        doctorSpecialization: response.data.chatDetails.doctorSpecialization,
      ...
      const sortedMessages = messages?.sort((a, b) => 
        new Date(a.MessageSentAt).getTime() - new Date(b.MessageSentAt).getTime()
      );
  ...
  const isDoctorMessage = (role: Message['MessageSenderType']) => role === 'doctor';
  const isPatientMessage = (role: Message['MessageSenderType']) => role === 'patient';

  const renderMessage = ({ item }: { item: Message }) => (
    <View
      style={[
        styles.messageContainer,
        isDoctorMessage(item.MessageSenderType) ? styles.doctorMessage : styles.patientMessage,
    ...
    >
      ...
      <View
        style={[
          ...
          {
            backgroundColor: isDoctorMessage(item.MessageSenderType) ? colors.patientPrimary : colors.background,
          },
        ]}
      >
        <ThemedText
          style={[
            styles.messageText,
            { color: isDoctorMessage(item.MessageSenderType) ? colors.background : colors.text },
          ]}
        >
          {item.MessageContent}
        </ThemedText>
        <View style={styles.messageFooter}>
          <ThemedText
            style={[
              styles.messageTime,
              { color: isDoctorMessage(item.MessageSenderType) ? colors.background + 'CC' : colors.textLight },
            ]}
          >
            {new Date(item.MessageSentAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
          </ThemedText>
    ...
    >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerInfo}>
            ...
            <View>
              ...
              <ThemedText style={[styles.headerSubtitle, { color: colors.background + 'CC' }]}>
      ...
      {isTyping && (
        <View style={styles.typingIndicator}>
          <ThemedText style={[styles.typingText, { color: colors.textLight }]}>
            Doctor is typing...
          </ThemedText>
        </View>
      )}
      ...
      <View style={[styles.inputContainer, { backgroundColor: colors.background }]}>
        ...
        <TouchableOpacity
          style={[
            styles.sendButton,
            {
              backgroundColor: message.trim().length > 0 ? colors.patientPrimary : colors.textLight,
            },
          ]}
          onPress={sendMessage}
