...
const ChatScreen = () => {
    ...
    useEffect(() => {
        const initializeChat = async () => {
            ...
            if (token) {
                ...
                const unsubscribeNewMessage = chatService.onNewMessage(
                    (message) => {
                        setChats((prevChats) => {
                            ...
                            const chatIndex = updatedChats.findIndex(
                                (c) =>
                                    c.ChatId ===
                                    message.MessageChatId.toString(),
                            );
                ...
                const unsubscribeReadReceipt = chatService.onReadReceipt(
                    (data) => {
                        setChats((prevChats) => {
                            const updatedChats = [...prevChats];
                            const chatIndex = updatedChats.findIndex(
                                (c) => c.ChatId === data.userId,
                            );
    ...
    const fetchChats = async () => {
        try {
            ...
            const chats = response.data.chats.map((chat: any) => ({
                ...
                ChatPatientSpecialization:
                    chat.patientSpecialization || "General",
    ...
    const filteredChats = chats.filter(
        (chat) =>
            chat.ChatPatientName.toLowerCase().includes(
                searchQuery.toLowerCase(),
            ) ||
            chat.ChatPatientSpecialization.toLowerCase().includes(
                searchQuery.toLowerCase(),
            ),
    );
        ...
        >
            ...
            <View style={styles.chatInfo}>
                <View style={styles.chatHeader}>
                    ...
                    >
                        {new Date(item.ChatTimestamp).toLocaleTimeString(
                            "en-US",
