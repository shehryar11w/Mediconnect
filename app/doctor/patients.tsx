...
const PatientsScreen = () => {
  ...
  const fetchPatients = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await dashboardService.getAllPatients(currentUser.id);
      ...
      if (response.success) {
        const formattedPatients = response.data.map((patient: any) => ({
          ...
          nextAppointment: patient.NextAppointmentDate ? new Date(patient.NextAppointmentDate).toLocaleDateString() : undefined,
          status: 'Active',
        }));
  ...
  const filteredPatients = patients.filter(patient => {
    const matchesSearch = patient.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         patient.condition.some(condition => condition.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <View style={styles.patientHeader}>
        <View style={styles.patientInfo}>
          ...
          <View style={styles.patientMeta}>
            <ThemedText style={[styles.patientMetaText, { color: colors.textLight }]}>
      ...
      <View style={styles.patientDetails}>
        ...
        {patient.nextAppointment && (
          <View style={styles.detailRow}>
            <Ionicons name="calendar-outline" size={16} color={colors.doctorPrimary} />
            <ThemedText style={[styles.detailText, { color: colors.doctorPrimary }]}>
          ...
          {(['all', 'active', 'inactive'] as const).map((filter) => (
            <TouchableOpacity
              ...
              style={[
                ...
                {
                  backgroundColor:
                    selectedFilter === filter ? colors.doctorPrimary : colors.background,
                },
              ]}
      ...
      <ScrollView style={styles.patientsContainer}>
        {isLoading ? (
          ...
          <View style={[styles.emptyState, { backgroundColor: colors.background }]}>
            ...
            <ThemedText style={[styles.emptyStateText, { color: colors.error }]}>
              {error}
            </ThemedText>
            <TouchableOpacity
