...
const Dashboard = () => {
  ...
  async function fetchData() {
    try{
    let res = await dashboardService.getTodaysAppointments(currentUser?.id);
  ...
  const metrics = [
    { title: 'Today\'s Appointments', value: todaysAppointments.length, icon: 'calendar' },
    { title: 'Total Patients', value: totalPatients, icon: 'people' },
  ];

  const quickActions = [
    { title: 'View Feedbacks', icon: 'chatbubble', route: '/doctor/feedback' },
    { title: 'View Analytics', icon: 'document-text', route: '/doctor/analytics' },
  ];
  ...
  const renderQuickAction = (action: typeof quickActions[0]) => (
    ...
    >
      <View style={[styles.quickActionIcon, { backgroundColor: colors.doctorPrimary + '20' }]}>
        <Ionicons name={action.icon as any} size={24} color={colors.doctorPrimary} />
      </View>
    ...
    >
      ...
      >
        <View style={styles.header}>
          ...
          <View style={styles.headerButtons}>
            ...
            <TouchableOpacity 
              style={[styles.headerButton, { backgroundColor: colors.background + '20' }]}
              onPress={() => router.push('/doctor/profile/profile' as any)}
            >
      ...
      <View style={styles.metricsContainer}>
        {metrics.map((metric, index) => (
          <View key={index} style={[styles.metricCard, { backgroundColor: colors.background }]}>
            <View style={styles.metricHeader}>
              <View style={[styles.metricIcon, { backgroundColor: colors.doctorPrimary + '20' }]}>
                <Ionicons name={metric.icon as any} size={24} color={colors.doctorPrimary} />
              </View>
             
            </View>
            <ThemedText style={styles.metricValue}>{metric.value}</ThemedText>
            <ThemedText style={[styles.metricTitle, { color: colors.textLight }]}>
              {metric.title}
            </ThemedText>
          </View>
        ))}
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          >
            <ThemedText style={[styles.seeAll, { color: colors.doctorPrimary }]}>View All</ThemedText>
            <Ionicons name="chevron-forward" size={16} color={colors.doctorPrimary} />
          </TouchableOpacity>
        </View>

        {upcomingAppointments.length > 0 ? upcomingAppointments.map((appointment, index) => (
          <View key={index} style={[styles.appointmentCard, { backgroundColor: colors.background }]}>
            <View style={[styles.timeContainer, { backgroundColor: colors.doctorPrimary }]}>
              <ThemedText style={[styles.time, { color: colors.background }]}>
                {formatTime(appointment.AppointmentStartDateTime)}
              </ThemedText>
            </View>
            <View style={styles.appointmentInfo}>
              <ThemedText style={styles.patientName}>{appointment.patient.PatientName}</ThemedText>
            </View>
            <View style={[styles.statusBadge, { 
              backgroundColor: appointment.AppointmentStatus === 'Confirmed' ? '#4CAF50' + '20' : '#FFA000' + '20'
            }]}>
              <ThemedText style={[styles.statusText, { 
                color: appointment.AppointmentStatus === 'Confirmed' ? '#4CAF50' : '#FFA000'
              }]}>
          ...
          <ThemedText style={styles.noAppointments}>No appointments today</ThemedText>
        )}
      </View>
      ...
      <View style={styles.section}>
        ...
        {recentPatients.length > 0 ? recentPatients.map((patient, index) => (
          <TouchableOpacity 
            ...
            onPress={() => router.push(`/doctor/patient/${patient.PatientId}`)}
          >
          ...
          <ThemedText style={styles.noAppointments}>No recent patients</ThemedText>
        )}
      </View>
    </ScrollView>
  );
