    ...
    try {
      ...
      if (response.success) {
        const formattedNotes = response.data.map((note: any) => ({
          ...
          category: note.NoteStatus.charAt(0).toUpperCase() + note.NoteStatus.slice(1),
        }));
  ...
  const renderNoteCard = (note: Note) => (
    ...
    >
      <View style={styles.noteHeader}>
        ...
        >
          <ThemedText style={[styles.categoryText, { color: colors.doctorPrimary }]}>
            {note.category}
          </ThemedText>
        </View>
        <ThemedText style={[styles.noteDate, { color: colors.textLight }]}>
          {note.date}
        </ThemedText>
      </View>
      ...
      <ScrollView style={styles.content}>
        {isAdding ? (
          <View style={[styles.addCard, { backgroundColor: colors.background }]}>
            <ThemedText style={styles.addTitle}>New Note</ThemedText>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.categoryButtons}>
                {(['General', 'Symptoms', 'Treatment', 'Follow-up'] as const).map((category) => (
                  <TouchableOpacity
                    key={category}
                    style={[
                      ...
                      {
                        backgroundColor:
                          newNote.category === category ? colors.doctorPrimary : colors.background,
                        borderColor: validationErrors.category ? colors.error : colors.textLight,
                      },
                    ]}
                    onPress={() => {
                  ...
                  >
                    <ThemedText
                      style={[
                        ...
                        {
                          color:
                            newNote.category === category ? colors.background : colors.text,
                        },
                      ]}
                    >
                      {category}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={[
                  ...
                  { 
                    color: colors.text,
                    borderColor: validationErrors.content ? colors.error : colors.textLight,
                  },
                ]}
            ...
            <View style={styles.buttonGroup}>
              <TouchableOpacity
                style={[styles.button, styles.cancelButton, { backgroundColor: colors.textLight }]}
                onPress={() => {
                  setIsAdding(false);
                  setValidationErrors({});
                  setBackendError(null);
