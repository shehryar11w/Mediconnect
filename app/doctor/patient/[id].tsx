...
const PatientScreen = () => {
  ...
  const fetchPatient = useCallback(async () => {
    try {
      setIsLoading(true);
      console.log(id);
      const details = await detailsService.getPatientDetails(parseInt(id as string));
      const medications = await prescriptionService.DoctorGetAllPrescriptions(parseInt(id as string));
      ...
      if (details.success) {
        ...
        setPatient({
          ...
          lastVisit: patientData.details.PatientLastVisit || 'No previous visits',
          nextAppointment: patientData.details.PatientNextAppointment,
          status: 'Active',
          bloodGroup: patientData.details.PatientBloodGroup || 'Not specified',
          allergies: patientData.details.PatientAllergies || [],
          medications: medications.data.map((medication: any) => medication.PrescriptionMedicine) || [],
          notes: notes.data.map((note: any) => note.NoteContent) || [],
        });
      } else {
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={[styles.card, { backgroundColor: colors.background }]}>
          <View style={styles.patientHeader}>
            <View style={styles.patientInfo}>
              ...
              <ThemedText style={[styles.patientMeta, { color: colors.textLight }]}>
            ...
            <View
              style={[
                ...
                {
                  backgroundColor:
                    patient.status === 'Active' ? '#4CAF50' + '20' : '#FFA000' + '20',
                },
              ]}
            >
              <ThemedText
                style={[
                  styles.statusText,
                  {
                    color: patient.status === 'Active' ? '#4CAF50' : '#FFA000',
                  },
                ]}
              >
                {patient.status}
        ...
        <View style={[styles.card, { backgroundColor: colors.background }]}>
          ...
          <View style={styles.section}>
            <ThemedText style={[styles.sectionLabel, { color: colors.textLight }]}>
              Current Conditions
            </ThemedText>
              ...
              {patient.conditions.length > 0 ? (
                patient.conditions.map((condition, index) => (
                  <View key={index} style={styles.conditionItem}>
                    ...
                    <ThemedText style={[styles.conditionText, { color: colors.textLight }]}>
                      {condition}
                    </ThemedText>
                  </View>
                ))
              ...
              {patient.medications.length > 0 ? (
                patient.medications.map((medication, index) => (
                  <View key={index} style={styles.medicationItem}>
                    <Ionicons name="medical" size={16} color={colors.textLight} />
                    <ThemedText style={[styles.medicationText, { color: colors.textLight }]}>
                      {medication}
                    </ThemedText>
                  </View>
                ))
              ...
              {patient.notes.length > 0 ? (
                patient.notes.map((note, index) => (
                  <View key={index} style={styles.noteItem}>
                    ...
                    <ThemedText style={[styles.noteText, { color: colors.textLight }]}>
                      {note}
                    </ThemedText>
                  </View>
                ))
